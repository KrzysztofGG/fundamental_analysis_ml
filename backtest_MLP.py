...
def fetch_rf_quarterly(start_date: str, end_date: str) -> pd.Series:
    """
    Returns quarterly risk-free rates derived from the 13-week T-bill yield (^IRX).
...
def retrain_on_batch_mlp(
    ...
    for i, (model, scaler, feats) in enumerate(
        ...
        orig_lr = model.lr
        model.set_params(max_epochs=max_epochs_finetune,
...
def run_backtest(
    ...
    for i, q in enumerate(tqdm(quarters[:-1], desc="Quarters")):
        ...
        if rebalance_mode == "fixed" and prev_batch is not None:
            model = retrain_on_batch_mlp(model, prev_batch, lower, upper, medians, meta)
